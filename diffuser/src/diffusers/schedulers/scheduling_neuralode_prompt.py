...
class NeuralODESchedulerPrompt(SchedulerMixin, ConfigMixin):
...
#         self.sigmas = torch.from_numpy(sigmas).to(device=device)
    ...
    def solver(
        ...
        def ODEFunc(t , x):
            ...
            x_value, embed_value = x

            latent_input = x_value / ((sigmas.item()**2 + 1) ** 0.5)
            latent_input = torch.cat([latent_input] * 2) if do_classifier_free_guidance else latent_input
            noise_pred = model(
                latent_input,
                steps,
                encoder_hidden_states=embed_value,
            ...
            #During the ODE solver process, just keep the prompt embed ''updates'' to be zero (keep the same along the process).
            embed_updates = torch.zeros_like(embed_value)
            return noise_pred, embed_updates

        # The adjoint_params set to be the prompt embeds. Here we take the prompt embeds as a part of input.
        # Combine the prompt_embeds into the input.  
        inputs = (model_input, prompt_embeds)
        solution = odeint(ODEFunc, inputs, self.sigmas, method=self.config.solver_type, adjoint_params=prompt_embeds)

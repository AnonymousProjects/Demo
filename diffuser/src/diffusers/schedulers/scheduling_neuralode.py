...
class NeuralODEScheduler(SchedulerMixin, ConfigMixin):
    ...
    def solver(
        ...
        # The adjoint_params set to be the model parameters, if we want to fine-tune the model
        solution = odeint(ODEFunc, model_input, self.sigmas, method=self.config.solver_type, adjoint_params=model.parameters())
